...
def get_parser(stages=['generate', 'compute']):
    ...
    parser.add_argument('--random_seed', type=int, default=10) # szhou: there is always the same random seed, so the same sample is sampled,, but how does that affect our downstream computations?
    parser.add_argument(
    ...
    if 'generate' in stages:
        ...
        parser.add_argument(
            ...
            choices=['trivia_qa', 'squad', 'bioasq', 'nq', 'svamp', 'paraphrase'],
            help="Dataset to use")
        parser.add_argument(
...
def construct_fewshot_prompt_from_indices(dataset, example_indices, brief, brief_always, make_prompt):
    ...
    for example_index in example_indices:
        ...
        question = example["question"] if "question" in dataset.features else example["questions"][-1]
