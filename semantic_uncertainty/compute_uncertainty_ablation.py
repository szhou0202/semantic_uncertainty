...
EXP_DETAILS = 'ablation_experiment_details.pkl'


def main(args):

    with open("data/all_paraphrase_answers.pkl", "rb") as generations_file:
    ...
    with open("data/uncertainty_measures_all_paraphrases.pkl", "rb") as uncertainty_measures_file:
    ...
    result_dict['valid_semantic_ids'] = []
    ...
    for idx, tid in enumerate(generations):
        if idx % 100 == 1:
            result_dict['is_false_fixed'] = [1.0 - np.exp(is_t) for is_t in is_true]
            result_dict['unanswerable'] = [1.0 - is_a for is_a in answerable]
            with open("data/combined_uncertainty_measures_paraphrases_ablation.pkl", "wb") as results_file:
        ...
        valid_full_responses = example["responses"] + example["valid_paraphrase_responses"]
        ...
        valid_responses = [fr[0] for fr in valid_full_responses]
        ...
        if args.compute_predictive_entropy:
            ...
            all_semantic_ids = get_semantic_ids(
                ...
                strict_entailment=args.strict_entailment, example=example)
            valid_semantic_ids = get_semantic_ids(
                valid_responses, model=entailment_model,
                strict_entailment=args.strict_entailment, example=example)

            result_dict['original_semantic_ids'].append(original_semantic_ids)
            result_dict['all_semantic_ids'].append(all_semantic_ids)
            result_dict['valid_semantic_ids'].append(valid_semantic_ids)
            ...
            entropies['valid_cluster_assignment_entropy'].append(cluster_assignment_entropy(valid_semantic_ids))
            ...
            valid_log_liks_agg = [np.mean(log_lik) for log_lik in valid_log_liks]
            ...
            entropies['valid_regular_entropy'].append(predictive_entropy(valid_log_liks_agg))
            ...
            valid_log_likelihood_per_semantic_id = logsumexp_by_id(valid_semantic_ids, valid_log_liks_agg, agg='sum_normalized')
            ...
            valid_pe = predictive_entropy_rao(valid_log_likelihood_per_semantic_id)

            entropies['original_semantic_entropy'].append(original_pe)
            entropies['all_semantic_entropy'].append(all_pe)
            entropies['valid_semantic_entropy'].append(valid_pe)
