...
def main(args):
    ...
    if args.train_wandb_runid is None:
        args.train_wandb_runid = args.eval_wandb_runid # szhou: g5zubouf sophia-zhou-yale-university/semantic_uncertainty/g5zubouf

    user = os.environ['USER']
    scratch_dir = os.getenv('SCRATCH_DIR', '.') # szhou: it will default to .
    wandb_dir = f'{scratch_dir}/{user}/uncertainty'
    slurm_jobid = os.getenv('SLURM_JOB_ID', None) # szhou: there is a slurm job id that changes for every codeserver session i init 
    project = "semantic_uncertainty" if not args.debug else "semantic_uncertainty_debug"

    logging.info('Reuse nonactive wandb id.')

    def restore(filename):
        class Restored:
            name = f'{wandb_dir}/wandb/run-20251119_144835-g5zubouf/files/{filename}' # szhou: generalize this 
        return Restored
