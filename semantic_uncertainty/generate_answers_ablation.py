...
def main(args):
    ...
    with open("data/generated_paraphrases/all_paraphrases.pkl", "rb") as file:
        generated_paraphrases = pickle.load(file)
    
    print("Number of paraphrased samples:", len(generated_paraphrases))
    # Subset to examples with 3 paraphrases (total, not just valid)
    has_three = []
    for k, v in generated_paraphrases.items():
        if len(v['paraphrases']) == 3:
            has_three.append(k)
    generated_paraphrases = {k: generated_paraphrases[k] for k in has_three}
    print("Samples left after removing those with fewer than 3 paraphrases:", len(generated_paraphrases))
    ...
    for dataset_split in ['train', 'validation']:
        ...
        for index in tqdm(indices):
            ...
            if (it % 100) == 1:
                ...
                with open('data/all_paraphrase_answers.pkl', 'wb') as gen_file:
                    pickle.dump(generations, gen_file)
                with open('data/uncertainty_measures_all_paraphrases.pkl', 'wb') as uncertain_file:
            ...
            if args.using_paraphrases:
                questions.extend(generated_paraphrases[example["id"]]['valid_paraphrases'])
                all_questions.extend(generated_paraphrases[example["id"]]['paraphrases'])

                generations[example["id"]].update({"question_and_paraphrases": questions})
                generated_paraphrases[example["id"]].update({"question_and_all_paraphrases": all_questions})
                ...
                for i in range(num_generations):
                    ...
                    if i == 0:
                        ...
                        if q_idx == 0: # Original question
                            ...
                            if q in questions: # is valid
                                valid_paraphrase_res.append((predicted_answer, token_log_likelihoods, embedding, acc))

            # Append all predictions for this example to `generations`.
            generations[example['id']]['responses'] = original_res
            generations[example['id']]['paraphrase_responses'] = paraphrase_res
            generations[example['id']]['valid_paraphrase_responses'] = valid_paraphrase_res
        ...
        if args.using_paraphrases:
            ...
            with open('data/all_paraphrase_answers.pkl', 'wb') as file:
        ...
        if dataset_split == 'validation' or args.using_paraphrases:
            ...
            if args.using_paraphrases:
                with open('data/uncertainty_measures_all_paraphrases.pkl', 'wb') as file:
